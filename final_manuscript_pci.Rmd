...
author:
  ...
  - firstname: Stefan
    ...
    affiliation: 1,2,10
lastauthor: 
  - firstname: Jennifer K. #add last author here if more than one author for proper styling
    lastname: Balch #can be kept empty/deleted otherwise
    affiliation: 1,2,6
address:
  - code: 1
    address: Earth Lab, University of Colorado, Boulder, CO
  - code: 2
    address: Department of Geography, University of Colorado, Boulder, CO
  - code: 3
    address: Water Resources, USDA-ARS, Fort Collins, CO
  - code: 4
    address: Department of Ecology and Evolutionary Biology, University of Colorado, Boulder, CO
  - code: 5
    address: Hydrostat, Inc., Washington, DC
  - code: 6
    address: Environmental Data Science Innovation and Inclusion Lab, University of Colorado, Boulder, CO
  - code: 7
    address: School of Forest, Fisheries, and Geomatic Sciences, Institute of Food and Agricultural Sciences, University of Florida, Gainesville, FL
  - code: 8
    address: Florida Sea Grant, Institute of Food and Agricultural Sciences, University of Florida, Gainesville, FL
  - code: 9
    address: Department of Human-Environment Systems, Boise State University, Boise, ID
  - code: 10
    address: Institute of Behavioral Science, University of Colorado, Boulder, CO

email: admahood\@gmail.com #the "\" before the "@" is needed to escape and compile properly!
abstract: |
...
recommender: Eric Goberville  

reviewers:  Jane Wyngaard and one anonymous reviewer

date: 17^th^ October 2022
pubyear: "`r format(Sys.time(), '%Y')`"
preprintserver: OSFpreprints
preprintversion: 6
# preprintnumber: 0
preprintDOI: 10.31219/osf.io/kehqz #important: "naked" doi, do NOT include the https://doi.org/ prefix here
recommendationDOI: 10.24072/pci.ecology.100102 #important: "naked" doi, do NOT include the https://doi.org/ prefix here

bibliography: references.bib 
# biblatexoptions: [backend=bibtex, maxcitenames=2]
#replace with path (relative or absolute) to your own
...
```{r,fig.cap="Different kinds of resolution, with examples of lower and higher resolution data. Spatial resolution relates to pixel size, temporal resolution to observation frequency, radiometric resolution to the number of unique values, and spectral resolution to binwidth in the electromagnetic spectrum."}

knitr::include_graphics("Figure1.png", dpi=375)
```
...
xl1  %>%
  dplyr::select(-Group) %>%
  kable(booktabs=T, caption = "Datasets mentioned in this article.")%>%
  kableExtra::kable_styling(latex_options = c("scale_down", "hold_position")) %>%
  kableExtra::group_rows(index = table(xl1$Group)) %>%
  column_spec(1:10, width = "10em")
...
High-resolution datasets are information-rich, with many potentially exciting science applications to explore. This supports new discoveries (see Allow for the unexpected), and methods (see Know when to innovate), but it can be easy to get distracted from the original  science question, lost in tangential, but exciting inquiries. While adjusting the scope may sometimes be beneficial, it is important to keep focus on the main goal regardless of whether it is to develop a new method or to investigate a particular phenomenon. Researchers might need to do both, but one should be the focus while the other plays a beneficial and supporting role during the research process.
...
xl2 %>%
  dplyr::select(-Group) %>%
  kable(booktabs=T, caption = "Practical and ethical resources.") %>%
  kableExtra::kable_styling(latex_options = c("scale_down", "hold_position"))%>%
...
xl3  %>%
  dplyr::select(-Group) %>%
  kable(booktabs=T, caption = "Software resources.") %>%
  kableExtra::kable_styling(latex_options = c("scale_down", "hold_position")) %>%
...
Preprint version 6 of this article has been peer-reviewed and recommended by Peer Community In Ecology (https://doi.org/10.24072/pci.ecology.100102)
